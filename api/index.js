  ...
  try {
    ...
    if (data.code === 0 && data.data) {
      return {
        ...
        video_url: data.data.play || data.data.wmplay,
        no_watermark: data.data.hdplay || data.data.play,
  ...
  try {
    ...
    if (data.success && data.data) {
      return {
        ...
        video_url: data.data.hd || data.data.sd,
...
async function downloadTelegram(url) {
  try {
    ...
    const { data } = await axios.get(embedUrl, { headers: { "User-Agent": "Mozilla/5.0" } });
  ...
  try {
    ...
    if (media || videoUrl?.includes("v.redd.it")) {
      return {
        platform: "reddit",
        title: post.title,
        video_url: media?.fallback_url || videoUrl,
